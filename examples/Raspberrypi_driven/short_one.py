...
nnBlobPath = str((Path(__file__).parent / Path('/home/pi/Open-Guide-OAK-D/examples/models/2classes_model.blob')).resolve().absolute())
if len(sys.argv) > 1:
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if(found_flag and speak_flag):
            ...
            t1 = gtts.gTTS(message)
            t1.save("ins.mp3")
            #Popen(opensound + 'ins.mp3', shell=True)
            #playsound("ins.mp3")
            os.system("mpg123 " + "ins.mp3")
            #engine.say(message)
            #engine.runAndWait()
            #tts = gTTS(text = message, lang='en')
            #tts.save('ins.mp3')
            #os.system("mpg321 ins.mp3")
            print("say")
            speak_flag = False
        count+=1
